...
<body>
        ...
        <div class="col-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3">
                    <h6 class="m-0 font-weight-bold text-black">All Products</h6>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-hover">
                            <thead>
                                <tr>
                                    <th style="background-color: #232323" class="text-white ">Product_Image</th>
                                    <th style="background-color: #232323" class="text-white ">Name</th>
                                    <th style="background-color: #232323" class="text-white ">Description</th>
                                    <th style="background-color: #232323" class="text-white ">Stock_Qty</th>
                                    <th style="background-color: #232323" class="text-white ">Cost_Price</th>
                                    <th style="background-color: #232323" class="text-white ">Selling_Price</th>
                                    <th style="background-color: #232323" class="text-white ">%Discount</th>
                                    <th style="background-color: #232323" class="text-white ">Category</th>
                                    <th style="background-color: #232323" class="text-white ">Size</th>
                                    <th style="background-color: #232323" class="text-white ">Status</th>
                                    <th style="background-color: #232323" class="text-white text-center">Specification</th>
                                    <th colspan=2 style="background-color: #232323" class="text-white text-center">Actions</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($all_products as $products)
                                <tr>
                                    <td><img class="w-75 h-75" src="{{ Storage::url($products->image) }}" alt=""></td>
                                    <td>{{ $products->productname }}</td>
                                    <td class="short-text">{{ $products->productdescription }}</td>
                                    <td>{{ $products->stockquantity ?? '0' }}</td>
                                    <td>Rs.{{ $products->costprice }}</td>
                                    <td>Rs.{{ $products->totalprice }}</td>
                                    <td>%{{ $products->discount }}</td>
                                    <td>{{ $products->category }}</td>
                                    <td>{{ $products->size }}</td>
                                    <td class="d-flex flex-row " style="width: 110px;">
                                        <select class="form-select-sm m-auto " width="50px" name="status" id="status">
                                    ...
                                    <td>
                                      <a href="{{ route('product-specifications.create', $products->id) }}" class="btn btn-sm btn-outline-success">+Specifications</a>
                                      <a href="{{ route('product-specifications.edit', $products->id) }}" class="btn btn-sm btn-outline-success">+ Edit pdt Spex</a>
                                    </td>

                                    <td>
                                      <a href="{{ route('product.edit', $products->id) }}" class="btn btn-sm btn-outline-success">Edit</a>
                                    </td>

                                    <td>
                                      <form action="{{ route('products.destroy', $products->id) }}" onsubmit="return confirm('Are you sure you want to delete this product?');" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-sm btn-outline-primary">Delete</button>
                                        <!-- Link to add specifications -->
                                      </form>
                                    </td>
                                </tr>
                                @endforeach
  ...
  <script>
    ...
    function shortenText(selector, maxWords) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
      shortenText('.short-text', 5); // Limit to 5 words
  });
