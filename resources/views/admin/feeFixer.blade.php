      ...
      <div>
        ...
        <nav class="sidebar-nav scroll-sidebar" data-simplebar="">
          <ul id="sidebarnav">
            ...
            <li class="sidebar-item">
              <a class="sidebar-link rounded-1 p-2" href="{{ route('feefixer.update', $feefix_update->id) }}" aria-expanded="false">
                <span>
                  <iconify-icon icon="solar:text-field-focus-bold-duotone" class="fs-6"></iconify-icon>
                </span>
    ...
    <div class="body-wrapper">
      ...
      <!--  Header End -->
        ...
        <div class="container-fluid">
            ...
            <h2 class="my-4">Fee Fixer</h2>
            
            <!-- Form to update fees -->
            @if(isset($feefix_update->id))
            <form action="{{ route('feefixer.update', $feefix_update->id) }}" method="POST">
                @csrf
                <div class="form-group inline">
                    @foreach($all_fee as $fee)
                        <div class="">
                            <div>
                                <label for="name">Current Tax Fee</label>
                                <input type="number" disabled value="{{ number_format($fee->tax_price ?? 0, 2) }}" name="tax_price" placeholder="Enter tax fee">
                            </div>
                        </div>
                        <div class="">
                            <div>
                                <label for="name">Current Shipping Fee</label>
                                <input type="number" disabled value="{{ $fee->shipping_cost ?? '0.00' }}" name="shipping_cost" placeholder="Enter shipping fee">
                            </div>
                        </div>
                    @endforeach
                    ...
                    <div class="">
                        <div>
                            <label for="name">Tax Fee</label>
                            <input type="number" id="tax_price" name="tax_price" placeholder="Enter tax fee">
                        </div>
                    </div>
                    <div class="">
                        <div>
                            <label for="name">Shipping Fee</label>
                            <input type="number" id="shipping_cost" name="shipping_cost" placeholder="Enter shipping fee">
                        </div>
                    </div>
                </div>
                <div class="d-flex flex-row"> 
                  <button type="submit" class="w-50 button m-auto text-white border border-dark" style="background-color: #232323; margin-left: 115px">Apply Charges</button>
                </div>
              </form>
              @else
                <p>Error: No fee fixer ID available for update.</p>
            @endif
              <div class="my-3">
                <p class="text-danger fw-bold">NOTE:&nbsp<span class="text-dark">If you want to remove the exixting Tax / Fee then first add the new one for safety purpose!</span></p>
                <form action="{{ route('feefixer.destroy', $feefix_update->id) }}" onsubmit="return confirm('Are you sure you want to delete this Fee?');" method="POST">
                  @csrf()
                  @method('DELETE')
                  <button type="submit" class="px-2 text-white border border-dark" style="background-color: #232323; margin-left: 115px">Remove the existing?</button>
                </form> 
              </div>
            </div>
          </div>
